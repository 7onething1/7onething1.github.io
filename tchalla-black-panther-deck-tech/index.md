...
**The Snail asterisk on the two premium swords.** Shadowspear and Swordsman's Steel are the strongest cards on the whole list, and by Snail's own wrong-choice principle that is exactly the reason to pause. They are raw power over identity. Swordsman's Steel is a four-mana artifact, so casting it does stack T'Challa's two counters, while Shadowspear at one mana sits below the mana-value-four line and does not. and a casual Wakanda table does not need a $56 upgrade to have its fun. Run them to punch through a grindy pod, and leave them in the box with a clear conscience if the deck is for kitchen-table nights. The rest of the package carries the plan without them.
...
| Vibranium Strike Gauntlets (MV4, artifact) | Suit up + draw on connect | Every Panther attack + 26 MV4 artifacts | Weak, wants a real body | **core** |
| Mystic Forge (MV4, artifact) | Card advantage off the top | The deck's 26+ artifacts | Yes, casts artifacts alone | **supported** |
| Zuri, Warrior of Wakanda (MV2) | Team-wide counter per big artifact cast | The 26 MV4 artifacts | Partial, idle until a qualifier | **supported** |
