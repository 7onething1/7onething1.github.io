...
## A peer warning, tested rather than adopted

The Keep It Greasey session reported that Songsterr renumbers element ids on a round trip, its own re-export moving Beats 1,527 to 1,556 and Notes 680 to 679 with zero content change. Checked here by breaking it on purpose: every Beat and Note id in the head file was shifted by +5000 with every reference rewritten, and the census re-run. **2,784 instances on each side, 0 added and 0 removed.** Every set difference in this session keys on (bar index, voice index, position in quarters, midi number), so no element id enters any comparison. Recorded as F6 in `FINDINGS.md`, and it carries into the upload step.
