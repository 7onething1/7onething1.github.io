...
## What went wrong
I ranked chats by the app's `lastActivityAt`. The babysit and digest launchd jobs
write to a session on their own schedule, so that field moves without Brandon
touching anything. It reports that a chat changed. It does not report that he used it.
...
## Why the count is still above 15
